...
class VisionQAApp:
    def __init__(self, root):
        self.root = root
        self.root.title("3D Obje → OpenSCAD Kodu Üretici")
    ...
    def find_openscad(self):
        ...
        # Windows'ta yaygın yollar
        possible_paths = [
            r"C:\Program Files\OpenSCAD\openscad.exe",
            r"C:\Program Files (x86)\OpenSCAD\openscad.exe",
            r"C:\OpenSCAD\openscad.exe",
            "openscad",  # PATH'te varsa
        ]
    ...
    def encode_image(self, image_path):
        ...
        # Görseli aç ve boyutlandır
        img = Image.open(image_path)
        
        # Maksimum boyut (API limitleri için)
        max_size = 1024
        
        # Görsel çok büyükse boyutlandır
        if img.width > max_size or img.height > max_size:
            img.thumbnail((max_size, max_size), Image.Resampling.LANCZOS)
        
        # JPEG formatına çevir ve optimize et
        ...
        # BytesIO kullanarak bellekte sakla
        buffer = BytesIO()
    ...
    def render_openscad(self):
        ...
        try:
            ...
            # OpenSCAD'ı çalıştır (PNG export)
            # --render: tam render (preview değil)
            # --imgsize: görsel boyutu (genişlik, yükseklik)
            # --viewall: tüm objeyi görüntüle
            # --autocenter: objeyi merkeze al
            cmd = [
                self.openscad_path,
                "--render",
                "--imgsize=800,600",
            ...
            if result.returncode != 0:
                ...
                # OpenSCAD syntax hatalarını daha okunabilir göster
                if "ERROR" in error_msg:
                    lines = error_msg.split('\n')
                    error_lines = [line for line in lines if 'ERROR' in line or 'WARNING' in line]
                    if error_lines:
                        error_msg = '\n'.join(error_lines[:5])  # İlk 5 hatayı göster
                ...
                if fixed_code and fixed_code != code:
                    ...
                    # Düzeltme başarısız oldu, hatayı göster
                    messagebox.showerror("Render Hatası", f"OpenSCAD render hatası:\n\n{error_msg}\n\nOtomatik düzeltme başarısız oldu. 'Otomatik Düzelt' butonunu manuel olarak deneyebilirsiniz.")
                    self.clear_render()
                    self.render_label.configure(text="Render hatası")
                    return
            
            # Render edilmiş görseli göster
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "ERROR" in error_msg:
                    ...
                    error_lines = [line for line in lines if 'ERROR' in line or 'WARNING' in line]
                    if error_lines:
                        error_msg = '\n'.join(error_lines[:10])  # İlk 10 hatayı göster
    ...
    def fix_syntax_error(self, code, error_message):
        """LLM'e hatayı gösterip düzeltmesini ister"""
        if not self.client:
            return None
...
"""
    ...
    def auto_fix_code(self):
        ...
        for attempt in range(max_attempts):
            ...
            # Hata varsa LLM'e gönder, düzeltmesini iste
            if error_message:
                ...
                if fixed_code and fixed_code != current_code:
                    ...
                    # Düzeltme yapılamadı veya aynı kod döndü
                    break
            else:
                break
    ...
    def show_render(self, path):
        ...
        try:
            img = Image.open(path)
            # Label boyutuna uygun şekilde ölçekle
            img.thumbnail((400, 400), Image.Resampling.LANCZOS)
            photo = ImageTk.PhotoImage(img)
            self.render_label.configure(image=photo, text="")
            self.render_label.image = photo  # type: ignore
            # Render başarılı oldu, iyileştir butonunu aktif et
            if self.image_path:  # İlk resim varsa iyileştirme yapılabilir
    ...
    def analyze_image(self):
        ...
        try:
            # Yeni kod üretilirken render alanını temizle
            self.clear_render()
...
""")
            ...
            final_prompt = "".join(user_prompt_parts)
            ...
            completion = self.client.chat.completions.create(
                ...
                max_tokens=4000,  # Uzun kod için yeterli
            )
            ...
            if answer:
                # Markdown code fence'leri temizle (model yine de eklerse)
    ...
    def show_prompt_debug(self):
        ...
        # analyze_image metodundaki prompt mantığını tekrar oluştur
        has_image = self.image_path is not None
        text_description = self.text_input.get(1.0, tk.END).strip()
        has_text = text_description and text_description != "Bu 3D objenin OpenSCAD kodunu üret. Boyutlar, şekiller ve detaylar hakkında açıklama yapabilirsiniz."
        question = self.question_entry.get().strip()
...
""")
        ...
        popup.title("🔍 Gönderilen Prompt (Debug)")
        popup.geometry("900x700")
        
        # System instruction göster
        sys_frame = tk.Frame(popup, bg="#f0f0f0")
        sys_frame.pack(fill="both", expand=True, padx=10, pady=5)
        
        tk.Label(sys_frame, text="SYSTEM INSTRUCTION:", bg="#f0f0f0", font=("Arial", 10, "bold"), fg="#1976D2").pack(anchor="w")
        sys_text = scrolledtext.ScrolledText(sys_frame, width=100, height=8, font=("Courier", 9), wrap=tk.WORD)
        sys_text.pack(fill="both", expand=True, pady=5)
        sys_text.insert(1.0, system_instruction)
        sys_text.configure(state="disabled")
        
        # User prompt göster
        user_frame = tk.Frame(popup, bg="#f0f0f0")
        user_frame.pack(fill="both", expand=True, padx=10, pady=5)
        
        tk.Label(user_frame, text="USER PROMPT:", bg="#f0f0f0", font=("Arial", 10, "bold"), fg="#388E3C").pack(anchor="w")
        user_text = scrolledtext.ScrolledText(user_frame, width=100, height=20, font=("Courier", 9), wrap=tk.WORD)
        user_text.pack(fill="both", expand=True, pady=5)
        user_text.insert(1.0, final_prompt)
        user_text.configure(state="disabled")
        
        # Bilgi
        info_label = tk.Label(popup, 
            text=f"✅ YENİ GELİŞTİRİLMİŞ PROMPT KULLANILIYOR\n📊 Görsel: {'Var' if has_image else 'Yok'} | Metin: {'Var' if has_text else 'Yok'} | Ek Talimat: {'Var' if question else 'Yok'}", 
            bg="#E8F5E9", 
            font=("Arial", 9, "bold"),
            fg="#2E7D32")
    ...
    def auto_render(self):
        ...
        if self.openscad_path:
            ...
            # OpenSCAD yoksa sadece bilgi ver
            self.render_label.configure(text="OpenSCAD bulunamadı\n(Manuel render için\nOpenSCAD yükleyin)")
            self.improve_button.configure(state="disabled")
    
    def improve_with_comparison(self):
        """Render edilmiş görseli ilk resimle karşılaştırarak iyileştir"""
        if not self.client:
            messagebox.showerror("Hata", "Önce token'ı kaydedin!")
            return
...
Output only the improved OpenSCAD code, starting with the code itself. Do not include explanations outside of code comments.""".format(current_code=current_code)
            
            # Mesaj içeriğini oluştur (iki görsel)
            content: list = [
